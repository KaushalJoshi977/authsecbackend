...
//				String key = entry.getKey();
				...
				if (value.equalsIgnoreCase("Shell Script")) {
					...
					script(prj_id, workflow_model, PRJ_NAME, portNumber, gitea_url, Deployment_profile);
					i++;
				}
...
//      CREATE SCRIPT FILE
	...
	public void script(Integer prj_id, String workflow_model, String prj, String portNumber, String gitea_url,
			Long deployment_line_id) throws IOException {
...
//		ArrayList<Object> list = calldeploymentprofile(Deployment_profile);
		...
		String sureops_dir = projectPath + File.separator + "cns-portal/code-extractor/builders" + File.separator + prj_id + File.separator
				+ "index";
...
//		jobdata.put("parameters", builder.toString());
		jobdata.put("url", "http://" + Constant.LOCAL_HOST + ":" + Constant.SUREOPS_PORT_9090
				+ "/sureops/runScript?filepath=" + filepath + "&filename=" + filename);
